...
export default function App() {
  ...
  // Додаємо Ref для правильного відстеження активної категорії всередині обробника скролу
  ...
  const handleOpenMenu = (section) => {
    ...
    if (firstCat) {
      ...
      activeCategoryRef.current = firstCat.id;
    }
    setSearchQuery('');
    setView('menu');
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const visibleCats = menuCategories.filter(c => c.section === menuSection);
      
      let newActive = activeCategoryRef.current;

      for (let i = visibleCats.length - 1; i >= 0; i--) {
      ...
      if (window.innerHeight + scrollY >= document.documentElement.scrollHeight - 30) {
        if (visibleCats.length > 0) {
          newActive = visibleCats[visibleCats.length - 1].id;
        }
      }

      if (newActive && newActive !== activeCategoryRef.current && newActive !== 'favorites') {
